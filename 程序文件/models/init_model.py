...
#from .architectures import FeatherNet
...
def get_model(net_type, clf_type, pretrained=False, output_size=2):
    ...
    elif net_type == 'InverNetDLAS_A':
        model = convergednet.invernetDLAS_A()
    elif net_type == 'InverNetDLAS_B':
        model = convergednet.invernetDLAS_B()
    elif net_type == 'FeatherNet_siam':         #自己的模型
        model = resnet_caffe.FeatherNet_siam(pretrained)    
    elif net_type =='FeatherNet':
        model = FeatherNet.FeatherNetBB()
    else:
